...
#define CREATE_DIM1  (new double[(a_NumCells+2)*(a_NumCells+2)])
 ...
 **********************************************************************/
ScalarField::ScalarField(int a_NumCells, double a_Viscosity, double a_Dt)
...
m_Viscosity(CopyField->m_Viscosity), m_Dt(CopyField->m_Dt)
{
    for(int i = 0; i < (m_NumCells+2)*(m_NumCells+2); i++)
 ...
 *
 * @param a_SrcField A scalar field representing the change in scalar values from user
 *        input to be added.
 * @param VelocityField The velocity field to advect the scalar through.
 ******************************************************************************/
void ScalarField::TimeStep(ScalarField *a_SrcField, VectorField *VelocityField)
{
    AddField(a_SrcField);
...
void ScalarField::Advection(VectorField *u)
{
    ...
    // top-right
    newField[IX_DIM(m_NumCells+1, m_NumCells+1)] 
        = newField[IX_DIM(m_NumCells, m_NumCells)];
