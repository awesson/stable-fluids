...
void VectorField::Advection(const VectorField& oldField)
{
	...
	ITER_DIM
		...
		newField[IX_DIM(i, j)] = oldField.Interpolate(prevPos);
	ENDITER_DIM
	...
	ITER_DIM
		...
		{
			m_Field[IX_DIM(i, j)][0] = 0.5 * (newField[IX_DIM(i, j)][0]
											+ newField[IX_DIM(i-1, j)][0]);
		}
		...
		{
			m_Field[IX_DIM(i, j)][1] = 0.5 * (newField[IX_DIM(i, j)][1]
											+ newField[IX_DIM(i, j-1)][1]);
		}
	ENDITER_DIM
...
void VectorField::Projection()
{
	...
	ITER_DIM
		divergence[IX_DIM(i, j)] = m_Field[IX_DIM(i+1, j)][0]
