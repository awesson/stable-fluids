...
namespace gfx
{
	...
	public:
		...
		//
		SymMat2(double s = 0.0)
		{
			*this = s;
		}

		SymMat2(const SymMat2 &m)
		{
			*this = m;
		}
	...
	{
		for (int i = 0; i < size(); i++) elt[i] = m.elt[i];
		return *this;
	}

	inline SymMat2& SymMat2::operator=(double s)
	{
		for (int i = 0; i < size(); i++) elt[i] = s;
		return *this;
	}
	...
	inline SymMat2 operator+(SymMat2 n, const SymMat2 &m)
	{
		n += m;
		return n;
	}
	...
	inline SymMat2 operator*(double s, SymMat2 m)
	{
		m *= s;
		return m;
	}

	inline SymMat2 operator*(SymMat2 m, double s)
	{
		m *= s;
		return m;
	}
	...
	inline double det(const SymMat2 &m)
	{
		return m(0, 0) * m(1, 1) - 2 * m(0, 1);
	}

	inline double trace(const SymMat2 &m)
	{
		return m(0, 0) + m(1, 1);
	}

	inline SymMat2 transpose(const SymMat2 &m)
	{
		return m;
	}
