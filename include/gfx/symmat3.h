...
namespace gfx
{
	...
	public:
		...
		//
		SymMat3(double s = 0.0)
		{
			*this = s;
		}

		SymMat3(const SymMat3 &m)
		{
			*this = m;
		}
	...
	inline SymMat3 operator+(SymMat3 n, const SymMat3 &m)
	{
		n += m;
		return n;
	}
	...
	inline SymMat3 operator*(double s, SymMat3 m)
	{
		m *= s;
		return m;
	}

	inline SymMat3 operator*(SymMat3 m, double s)
	{
		m *= s;
		return m;
	}
	...
	inline double det(const SymMat3 &m)
	{
		return m.row(0) * (m.row(1) ^ m.row(2));
	}

	inline double trace(const SymMat3 &m)
	{
		return m(0, 0) + m(1, 1) + m(2, 2);
	}

	inline SymMat3 transpose(const SymMat3 &m)
	{
		return m;
	}
